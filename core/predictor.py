# OMEGA_PRO_v10.1/core/predictor.py – HybridOmegaPredictor OMEGA PRO AI v10.1 – Versión Mejorada con Robustez Anti-Crash
...
# Nueva importación para calibración
...
class HybridOmegaPredictor:
    ...
    DEFAULT_DATA_PATHS = [
        ...
        "backup/historial_kabala_github_fixed.csv",
        "https://raw.githubusercontent.com/omega-pro-ai/historial/main/historial_kabala_github_fixed.csv"
    ]
    ...
    def __init__(
        ...
        self.data = clean_historial_df(self.data)  # Limpieza robusta

        if self.data.empty:
            self.logger.warning("⚠️ Historial vacío, generando dummy data")
            self.data = pd.DataFrame(np.random.randint(1, 41, size=(100, 6)), columns=[f'B{i}' for i in range(1,7)])
        ...
        # Contexto para almacenar información de depuración
        self.ctx = {}
    ...
    def cargar_datos(self) -> pd.DataFrame:
        ...
        if not path:
            self.logger.warning("⚠️ Data path inválido, usando datos dummy")
            return clean_historial_df(pd.DataFrame())  # Fallback limpio
        ...
        try:
            ...
            # Limpieza robusta de datos históricos
            df = clean_historial_df(df)
            
            # FIX: Manejo explícito de NaN e infinitos
            df = df.fillna(0).replace([np.inf, -np.inf], 0).astype(int)
            
            self.logger.info(f"📊 Datos cargados y limpiados: {df.shape[0]} sorteos")
            return df

        except Exception as e:
            self.logger.error("Error cargando datos", exc_info=True)
            raise DataLoadError(f"Error cargando datos desde {path}", path=path)
    ...
    def aplicar_minado_inverso(self, ultima_combinacion: List[int]) -> List[Dict[str, Any]]:
        ...
        # FIX: Manejar caso de combinación limpia None
        if clean_last is None or len(clean_last) != 6:
            self.logger.warning("⚠️ Ultima combinación inválida o None, skipping minado inverso")
            return []
    ...
    def filtrar_combinaciones(
        ...
        # 1. Obtener todas las combinaciones limpias para calcular el core_set
        clean_combinations = []
        ...
        # 2. Calcular el core_set con los números más frecuentes
        ...
        # 3. Crear filtro de cobertura
        cobertura_filter = CoberturaCore(core_set, min_hits=4, penalizar=False)
        ...
        for idx, item in enumerate(combinaciones):
            ...
            try:
                # Aplicar filtro de cobertura primero
                cobertura_ok, cobertura_score = cobertura_filter.apply(combo)
                
                # Solo aplicar otros filtros si pasa cobertura
                if cobertura_ok:
                    ...
                    score = 0.0  # Si falla cobertura, descartar
                    
            except Exception as e:
                continue
                
            threshold = {'moderado':0.7,'conservador':0.8,'agresivo':0.4}.get(
                self._map_svi_profile(self.perfil_svi),0.7)
            ...
            if score >= threshold:
                ...
                item["metrics"]["cobertura_score"] = cobertura_score  # Registrar métrica
                item["normalized"] = 0.0
                final.append(item)
    ...
    def _run_consensus(self, max_comb: int) -> List[Dict[str, Any]]:
        try:
        ...
        except TypeError as e:  # Fix para GeneticConfig arg error
            self.logger.error(f"🚨 Error en GeneticConfig: {e}. Usando defaults alternos.")
            ...
            # ... procesar raw como antes
            results = []
            for item in raw:
                pass  # TODO: implementar procesamiento
            return results
    ...
    def _run_lstm(self, max_comb: int) -> List[Dict[str, Any]]:
        try:
            # FIX: Convertir data a numpy array explícitamente
            data_array = np.array(self.data.values) if isinstance(self.data.values, list) else self.data.values
            ...
            raw = generar_combinaciones_lstm(
                data=data_array,  # Pasar array en lugar de lista
                cantidad=max_comb,
    ...
    def _run_transformer(self, max_comb: int) -> List[Dict[str, Any]]:
        try:
            ...
            return results
        except IndexError as e:  # Fix para out of bounds
            self.logger.error(f"🚨 Shape error in Transformer: {e}. Using fallback generation.")
            # Generar fallbacks manual
            results = []
            for _ in range(max_comb):
                clean = sorted(random.sample(range(1, 41), 6))
                results.append({
                    "combination": clean,
    ...
    def run_all_models(self) -> List[Dict[str, Any]]:
        ...
        # === Verificación de archivos de modelo requeridos ===
        required_files = [
            'models/gboost_model.pkl',
            'models/jackpot_profiler.pkl',
            'models/jackpot_profiler_mlb.pkl'
        ]
        for f in required_files:
            if not os.path.exists(f):
                self.logger.warning(f"⚠️ Archivo crítico {f} no encontrado; se usará fallback o entrenamiento dummy.")
        max_comb = 500
        if PSUTIL_AVAILABLE:
        ...
        try:
            ...
            self.logger.error(f"🚨 Error en dynamic scoring: {e}. Usando scoring secuencial fallback.")
            # Scoring secuencial simple como fallback
            for c in combinaciones:
                # Simple score calc
                dyn_score = random.uniform(0.5, 1.0)  # Placeholder, implementar real si posible
                svi_score = c.get("svi_score", 0.5)
                c["score"] = 0.6 * svi_score + 0.4 * dyn_score
                c.setdefault("metrics", {})["dynamic_score"] = dyn_score
            self.logger.info("✅ Dynamic scoring fallback aplicado")
        ...
        # --------------------------------------------------------------------
        # Paso A: calcular el core_set (los 6 números más frecuentes)
        # --------------------------------------------------------------------
        ...
        # --------------------------------------------------------------------
        # Paso C: aplicar filtros con CoberturaCore
        # --------------------------------------------------------------------
        # Filtrar combinaciones usando el core_set
        # (Nota: El filtro de cobertura ya está integrado en filtrar_combinaciones)
        ...
        # --------------------------------------------------------------------
        # Paso D: seleccionar combinación maestra
        # --------------------------------------------------------------------
        try:
            # Preparar formato (renombrar 'combination' a 'combinacion')
            ...
            # Agregar la combinación maestra a los resultados finales
            final.append({
        ...
        # --------------------------------------------------------------------
        # Paso E: recalibrar pesos de modelos
        # --------------------------------------------------------------------
        try:
            # Usar las combinaciones filtradas para recalibrar
            series_filtradas = combinaciones
