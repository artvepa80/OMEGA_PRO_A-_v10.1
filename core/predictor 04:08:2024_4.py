...
# Configuración inicial de logging (evitar duplicados)
logging.basicConfig(
    level=logging.INFO,
...
if len(logger.handlers) > 1:  # Evitar handlers duplicados
...
class HybridOmegaPredictor:
    ...
    def __init__(
        ...
        # Logger único para evitar duplicados
        self.logger = logging.getLogger("OmegaPredictor")
        if logger:
    ...
    def cargar_datos(self) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.warning("⚠️ Usando datos dummy de respaldo")
            return clean_historial_df(pd.DataFrame())  # Fallback limpio
