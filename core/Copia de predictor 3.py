# OMEGA_PRO_AI_v10.1/core/predictor.py – HybridOmegaPredictor OMEGA PRO AI v10.1 – Versión Corregida con Robustez Anti-Crash
...
from modules.score_dynamics import score_combinations, clean_combination, SCORE_CONFIG  # FIXED: Import SCORE_CONFIG
...
class HybridOmegaPredictor:
    ...
    def __init__(
        ...
        self.key_path = "logs/auth_key.key"  # IMPROVED: Configurable key_path
        ...
        # Integración de módulos adicionales
        self.retro_tracker = RetroTracker() if backtest else None
        self.perf_metrics = None
        if evaluate:
            self.perf_metrics = evaluate_model_performance(self.data)
        if retrain:
            self.retrain_models()

    def retrain_models(self):
        if self.perf_metrics and any(metric["accuracy"] < 0.7 for metric in self.perf_metrics):
            self.logger.info("⚙️ Reentrenando modelos debido a bajo rendimiento")
            auto_retrain(self.data)
    ...
    def aplicar_ghost_rng(
        ...
        for r in resultados_rng:
            ...
            if r['composite_score'] <= 0:  # IMPROVED: Validate composite_score >0
                continue
            valid.append(r)
    ...
    def _obtener_token(self) -> str:
        if not CRYPTO_AVAILABLE:
            self.logger.warning("⚠️ Cryptography no disponible, usando token simulado")
            return "simulated_token"
        if self._internal_token:
            return self._internal_token
        if not os.path.exists(self.key_path):  # IMPROVED: Use self.key_path
    ...
    def filtrar_combinaciones(
        ...
        # IMPROVED: Parallelize if >100 combos
        if len(combinaciones) > 100:
            self.logger.info("🧪 Paralelizando filtros...")
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                futures = [executor.submit(filtrar_item, item) for item in combinaciones]
                final = [f.result() for f in as_completed(futures) if f.result() is not None]
        else:
    ...
    def export_chart(self, chart_name: str, chart_type: str, chart_data: dict, output_dir: str = "results"):
        ...
        path = os.path.join(output_dir, f"{chart_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{os.getpid()}.json")  # IMPROVED: Add PID
    ...
    def _run_lstm(self, max_comb: int) -> List[Dict[str, Any]]:
        try:
            if self.data.shape[0] < self.lstm_config['min_history']:
                self.logger.warning("⚠️ Historia insuficiente para LSTM, usando fallback")  # IMPROVED: Log and fallback
                fallback = sorted(random.sample(range(1,41),6))
                return [{
                    "combination": fallback,
                    "source": "lstm_fallback",
                    "score": 0.5,
                    "metrics": {"reason": "insufficient_history"},
                    "normalized": 0.0
                }]
    ...
    def run_all_models(self) -> List[Dict[str, Any]]:
        ...
        combinaciones: List[Dict[str, Any]] = []
        # IMPROVED: Parallelize model execution
        with ThreadPoolExecutor(max_workers=4) as executor:  # Adjust workers based on CPU
            ...
            for name, fn, cnt in models:
                if self.usar_modelos.get(name, True):
                    ...
                    futures.append(executor.submit(fn, cnt))
            ...
            gc.collect()  # ADDED: Free memory after models
        ...
        # Minado inverso - FIXED: Flexible unpacking
        ...
        # Dynamic scoring (opcional) - FIXED: Use multiprocessing.Pool to avoid joblib pickling issues
        try:
            def score_wrapper(args):
                combinations_chunk, historial, cluster_data, config, perfil_svi, logger = args
                return score_combinations(combinations_chunk, historial, cluster_data, config, perfil_svi, logger)

            chunk_size = max(1, len(combinaciones) // 4)
            chunks = [combinaciones[i:i+chunk_size] for i in range(0, len(combinaciones), chunk_size)]
            args_list = [(chunk, self.data, None, SCORE_CONFIG, self._map_svi_profile(self.perfil_svi), self.logger) for chunk in chunks]

            with mp.Pool(processes=4) as pool:
                scored_chunks = pool.map(score_wrapper, args_list)

            scored = [item for chunk in scored_chunks for item in chunk]
        ...
        # 1) Clasificación GBoost Jackpot - FIXED: Fit with dummy if not fitted
        if self.usar_modelos.get("gboost", True):
            try:
                clf = GBoostJackpotClassifier()
                if not clf.is_fitted():
                    self.logger.warning("⚠️ GBoost not fitted, fitting with dummy data")
        ...
        # 2) Perfilado de Jackpot - FIXED: Sort with key
        if self.usar_modelos.get("profiling", True):
            try:
                ...
                # FIXED: Sort with key to avoid dict comparison
                perfil_metrics = sorted(perfil_metrics, key=lambda d: d.get('jackpot_prob', 0), reverse=True)
        ...
        # 3) Evaluador Inteligente - FIXED: Use 'evaluate' method
        if self.usar_modelos.get("evaluador", True):
            try:
                ...
                for combo in combinaciones:
                    ...
                    try:
                        result = evaluator.evaluate(combo["combination"])  # FIXED: Use 'evaluate'
