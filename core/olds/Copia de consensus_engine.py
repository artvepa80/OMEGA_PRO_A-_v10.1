# consensus_engine.py – Versión v12.4 FULL con validación mejorada, fallback y compatibilidad con predictor.py
...
from modules.transformer_model import generar_combinaciones_transformer  # Updated to match predictor.py
...
# Flags para activar o desactivar modelos, aligned with predictor.py
USE_MONTECARLO = True
USE_LSTM = True
USE_CLUSTERING = True
USE_GENETICO = True  # Changed to True to align with predictor.py and main.py
USE_RNG = True
...
def generar_combinaciones_consenso(
    ...
    try:
        ...
        if USE_MONTECARLO:
            ...
            try:
                historial_list = historial_df.values.tolist()
        ...
        if USE_GENETICO:
            ...
            try:
                historial_set = set(tuple(sorted(map(int, row))) for row in historial_df.to_numpy())
        ...
        if USE_APRIORI:
            ...
            try:
                ...
                resultados = generar_combinaciones_apriori(
                    ...
                    historial_set=historial_set,
                    cantidad=cantidades_por_modelo["apriori"],
                    logger=logger
                )
                valid_resultados = [r for r in resultados if validate_combination(r.get("combination", []))]
                if len(valid_resultados) < len(resultados):
